...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // if (client.connect(clientId.c_str(), mqtt_username, mqtt_password)) {
      ...
      client.publish("motionSensorLabor", "hello world");
...
void loop() {
  ...
  client.loop();
  handlePIROutput(digitalRead(pirPin));
  blink();
  buzz();
}
...
void publishMotionState() {
  if (motionState) {
    client.publish("motionSensorLabor", "true");
    Serial.println("Publish message: true");
  } else {
    client.publish("motionSensorLabor", "false");
